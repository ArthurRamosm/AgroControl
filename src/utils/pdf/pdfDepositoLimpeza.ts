...
function getItem(r: RegistroDeposito | undefined, n: number): string | null {
  if (!r) return null;
...
function buildHtml(
  ...
  mes: number,
  ano: number,
  registros: RegistroDeposito[]
): string {
  const mesLabel = MESES_FULL[mes - 1];

  const porSemana: Record<number, RegistroDeposito> = {};
...
<h2>PL 03/05 – AVALIAÇÃO DAS CONDIÇÕES DO DEPÓSITO DE EMBALAGENS E MATERIAIS DE LIMPEZA</h2>
...
export async function gerarPdfDepositoLimpeza(
  mes: number,
  ano: number,
  ...
  try {
    ...
    if (podeCompartilhar) {
      await Sharing.shareAsync(uri, {
        ...
        dialogTitle: `PL 03/05 – Depósito Limpeza ${pad2(mes)}/${ano}`,
        UTI: 'com.adobe.pdf',
      });
    }
  } finally {
