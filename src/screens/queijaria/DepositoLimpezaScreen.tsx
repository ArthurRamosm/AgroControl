...
import { gerarPdfDepositoLimpeza } from '../../utils/pdf/pdfDepositoLimpeza';
...
const ITENS = [
  'Ambiente do depósito está limpo e organizado?',
  'Há presença de lixeira com acionamento por pedal?',
  'Há prateleiras suficientes?',
  'Há separação física entre o material de limpeza e as embalagens?',
  'O sal é acondicionado em tambores e em estrados?',
];
...
function recordToForm(r: DepositoRecord): WeekForm {
  return {
...
export default function DepositoLimpezaScreen() {
  ...
  const [monthRecords, setMonthRecords] = useState<Record<number, DepositoRecord>>({});
  ...
  const loadMonth = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const rows = db.getAllSync<DepositoRecord>(
        `SELECT * FROM pl_deposito_limpeza WHERE mes=? AND ano=? ORDER BY semana ASC`,
        [selectedMonth + 1, selectedYear]
      );
      const map: Record<number, DepositoRecord> = {};
      ...
      console.error('Erro ao carregar depósito limpeza:', e);
    } finally {
      setLoading(false);
    }
  ...
  async function handleSave() {
    ...
    try {
      ...
      const [i1, i2, i3, i4, i5] = wf.items;

      if (rec?.id) {
        db.runSync(
          `UPDATE pl_deposito_limpeza SET
            data_avaliacao=?,
            item_1=?,item_2=?,item_3=?,item_4=?,item_5=?,
            observacoes=?,acoes_corretivas=?,responsavel=?,
            sync_status='pending'
          WHERE id=?`,
          [wf.data_avaliacao, i1, i2, i3, i4, i5,
           wf.observacoes, wf.acoes_corretivas, wf.responsavel, rec.id]
        );
      } else {
        db.runSync(
          `INSERT INTO pl_deposito_limpeza
            (propriedade_id,mes,ano,semana,data_avaliacao,
             item_1,item_2,item_3,item_4,item_5,
             observacoes,acoes_corretivas,responsavel)
          VALUES (1,?,?,?,?,?,?,?,?,?,?,?,?)`,
          [mes, selectedYear, selectedWeek, wf.data_avaliacao,
           i1, i2, i3, i4, i5,
           wf.observacoes, wf.acoes_corretivas, wf.responsavel]
        );
      }
      await loadMonth();
      Alert.alert('Salvo', `Semana ${selectedWeek} de ${MESES[selectedMonth]}/${selectedYear} salva.`);
    } catch (e) {
  ...
  const handleGerarPdf = async () => {
    try {
      await gerarPdfDepositoLimpeza(selectedMonth + 1, selectedYear, 1);
    } catch (e) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View>
          <Text style={styles.headerTitle}>Depósito de Embalagens e Limpeza</Text>
          <Text style={styles.headerSub}>PL 03/05 — Avaliação semanal</Text>
        </View>
      </View>
