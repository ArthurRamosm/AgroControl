...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'ReceitaDetalhe'>;
};
...
export default function ReceitaDetalheScreen({ navigation }: Props) {
  const insets = useSafeAreaInsets();
  const { propriedadeId } = getSession();
  const now = new Date();
  const [mes, setMes] = useState(now.getMonth() + 1);
  const [ano, setAno] = useState(now.getFullYear());
  const [receitas, setReceitas] = useState<ReceitaDetalhe[]>([]);
  const [modalVisible, setModalVisible] = useState(false);
  const [editando, setEditando] = useState<ReceitaDetalhe | null>(null);
  ...
  const carregar = useCallback(() => {
    ...
    try {
      const rows = dbLocal.getAllSync(
        `SELECT * FROM receita_detalhe WHERE propriedade_id = ? AND data_venda LIKE ? ORDER BY data_venda DESC`,
  ...
  function abrirNovo() {
    setEditando(null);
    setForm({ ...formInicial, data_venda: hojeBr() });
    setModalVisible(true);
  }

  function abrirEditar(r: ReceitaDetalhe) {
    setEditando(r);
    setForm({
      data_venda: isoToBr(r.data_venda),
      data_recebimento: isoToBr(r.data_recebimento),
  ...
  function salvar() {
    ...
    if (!validarDataCompleta(form.data_venda)) { Alert.alert('Atenção', 'Data da venda inválida (DD/MM/AAAA).'); return; }
    if (form.data_recebimento && !validarDataCompleta(form.data_recebimento)) { Alert.alert('Atenção', 'Data de recebimento inválida (DD/MM/AAAA).'); return; }
    ...
    try {
      if (editando) {
        dbLocal.runSync(
          `UPDATE receita_detalhe SET data_venda=?, data_recebimento=?, descricao=?, quantidade=?, valor_unitario=?, valor_total=?, sync_status='pending' WHERE id=?`,
          [brToIso(form.data_venda), form.data_recebimento ? brToIso(form.data_recebimento) : null, form.descricao.trim(), qtd, vu, vt, editando.id]
        );
      } else {
        dbLocal.runSync(
          `INSERT INTO receita_detalhe (propriedade_id, data_venda, data_recebimento, descricao, quantidade, valor_unitario, valor_total, sync_status) VALUES (?,?,?,?,?,?,?,'pending')`,
          [propriedadeId, brToIso(form.data_venda), form.data_recebimento ? brToIso(form.data_recebimento) : null, form.descricao.trim(), qtd, vu, vt]
        );
      }
  ...
  function excluir(id: number) {
    Alert.alert('Excluir', 'Deseja excluir esta receita?', [
      { text: 'Cancelar', style: 'cancel' },
      { text: 'Excluir', style: 'destructive', onPress: () => {
        try { dbLocal?.runSync('DELETE FROM receita_detalhe WHERE id=?', [id]); carregar(); } catch {}
      }},
    ]);
  }

  const totalRecebido = receitas.filter(r => r.data_recebimento).reduce((s, r) => s + r.valor_total, 0);
  const totalAReceber = receitas.filter(r => !r.data_recebimento).reduce((s, r) => s + r.valor_total, 0);
      ...
      <ScrollView style={s.scroll} contentContainerStyle={{ paddingBottom: 40 + insets.bottom }}>
        ...
        <View style={s.header}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            <Text style={s.titulo}>Outras Receitas</Text>
        ...
        <View style={s.card}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 }}>
            <View>
              ...
              <Text style={s.totalValor}>R$ {fmt(totalRecebido)}</Text>
            </View>
            <View style={{ alignItems: 'flex-end' }}>
              <Text style={s.totalLabel}>A receber</Text>
              <Text style={[s.totalValor, { color: ACCENT }]}>R$ {fmt(totalAReceber)}</Text>
            </View>
          </View>
        </View>

        <View style={s.card}>
          <Text style={s.cardTitulo}>Lançamentos ({receitas.length})</Text>
          {receitas.length === 0 ? (
            <Text style={s.vazio}>Nenhuma receita registrada neste período.</Text>
          ) : receitas.map(r => (
            <View key={r.id} style={s.item}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <View style={{ flex: 1 }}>
                  <Text style={s.itemDesc}>{r.descricao}</Text>
                  <Text style={s.itemSub}>
                    Venda: {isoToBr(r.data_venda)}{r.data_recebimento ? ` · Recebido: ${isoToBr(r.data_recebimento)}` : ' · Pendente'}
                  </Text>
                  <Text style={s.itemCalc}>
                    {r.quantidade} × R$ {fmt(r.valor_unitario)} = <Text style={{ fontWeight: '800' }}>R$ {fmt(r.valor_total)}</Text>
                  </Text>
                  {!r.data_recebimento && (
                    <View style={s.badgePendente}>
                      <Text style={s.badgePendenteTxt}>A receber</Text>
                    </View>
                  )}
                </View>
                <View style={{ gap: 6 }}>
                  <TouchableOpacity onPress={() => abrirEditar(r)} style={s.btnEdit}>
                    <Text style={s.btnEditTxt}>Editar</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => excluir(r.id)} style={s.btnDel}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <View style={s.modalFundo}>
            <ScrollView style={s.modalCaixa} contentContainerStyle={s.modalContent} keyboardShouldPersistTaps="handled">
              <Text style={s.modalTitulo}>{editando ? 'Editar Receita' : 'Nova Receita'}</Text>

              <FLabel>Data da venda</FLabel>
              <FInput value={form.data_venda} onChangeText={v => setForm(f => ({ ...f, data_venda: formatarDataBrasileira(v) }))} keyboardType="number-pad" maxLength={10} placeholder="DD/MM/AAAA" />

              <FLabel>Data do recebimento (opcional)</FLabel>
              <FInput value={form.data_recebimento} onChangeText={v => setForm(f => ({ ...f, data_recebimento: formatarDataBrasileira(v) }))} keyboardType="number-pad" maxLength={10} placeholder="DD/MM/AAAA" />

              <FLabel>Descrição</FLabel>
              <FInput value={form.descricao} onChangeText={v => setForm(f => ({ ...f, descricao: v }))} placeholder="Ex: Venda de leite" />
...
const s = StyleSheet.create({
  ...
  totalLabel: { color: '#667766', fontSize: 12, fontWeight: '600' },
  totalValor: { color: PRIMARY, fontSize: 22, fontWeight: '900' },
  ...
  badgePendente: { backgroundColor: '#fff3e0', borderRadius: 6, alignSelf: 'flex-start', paddingHorizontal: 8, paddingVertical: 3, marginTop: 5 },
  badgePendenteTxt: { color: ACCENT, fontSize: 11, fontWeight: '700' },
